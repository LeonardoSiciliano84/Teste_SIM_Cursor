...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  const addFelkaHeaderAndFooter = (doc: any, title: string) => {
    try {
      ...
      const headerImagePath = path.join('assets', 'felka-header.png');
  ...
  // ============= CONTROLE DE ACESSO - CPF E QR CODE =============
  ...
  app.post("/api/access-control/visitors", async (req, res) => {
    try {
      ...
      const visitor = await storage.upsertVisitor(visitorData);
      res.json(visitor);
    } catch (error) {
      if (error instanceof z.ZodError) {
        return res.status(400).json({ message: "Dados inválidos", errors: error.errors });
  ...
  app.post("/api/employees/:id/qrcode", async (req, res) => {
    try {
      const { id } = req.params;
      ...
      // Gerar chave baseada em CPF
      const crypto = await import('crypto');
      const qrCodeData = crypto.createHash('sha256')
        .update(`${employee.cpf}-${employee.id}-${Date.now()}`)
        .digest('hex');

      const qrCode = await storage.createEmployeeQrCode({
        employeeId: id,
        cpf: employee.cpf,
        qrCodeData,
        isActive: true,
      ...
      res.json({ qrCode: qrCodeData, qrCodeId: qrCode.id });
    } catch (error) {
  ...
  // Processar QR Code (entrada/saída)
